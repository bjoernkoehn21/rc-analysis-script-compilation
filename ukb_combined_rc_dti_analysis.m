...
rcResults(:) = {struct( ...
	...
	'emp_max_phi_numerator', nan(length(FILES), length(EDGE_WEIGHTS)), ...
	'emp_max_phi_denominator', nan(length(FILES), length(EDGE_WEIGHTS)), ...
	'rand_max_phi_mean_rc_coefficient', nan(length(FILES), length(EDGE_WEIGHTS)), ...
    'integral', struct( ...
        'norm', nan(length(FILES), length(EDGE_WEIGHTS)), ...
        'emp', nan(length(FILES), length(EDGE_WEIGHTS)), ...
		'empRcRange', nan(length(FILES), length(EDGE_WEIGHTS)), ...
        'rand', nan(length(FILES), length(EDGE_WEIGHTS)), ...
		'randRcRange', nan(length(FILES), length(EDGE_WEIGHTS)), ...
...
function [rcResults] = compute_correct_assemble_rc_dti(PATH_TO_SUBJECT_DIRS, PATH_TO_FREESURFER_DIR, subjID, N_RANDOM_NETWORKS, EDGE_WEIGHTS, ...
    ...
    for iFile = 1:length(FILES)
        ...
        subsubfields, 1)}, length(subfields), 1), subfields, 1)}, length(EDGE_WEIGHTS), 1), EDGE_WEIGHTS, 1)};
            ...
            [rc{iFile}.fa.rand.phi, rc{iFile}.fa.rand.phiNum, rc{iFile}.fa.rand.phiDenom] = ...
                deal(nan(length(rc{iFile}.fa.emp.phi), N_RANDOM_NETWORKS));
            [rc{iFile}.svd.rand.phi, rc{iFile}.svd.rand.phiNum, rc{iFile}.svd.rand.phiDenom] = ...
        ...
        try
            for iEdgeWeight = 1:length(EDGE_WEIGHTS)
                ...
                rcResults.emp_max_phi_numerator(iFile,iEdgeWeight) = rc{iFile}.( ...
				...
				meanRandRcCoefficient = nanmean(rc{iFile}.(EDGE_WEIGHTS{iEdgeWeight}).rand.phi, 2);
                rcResults.integral.rand(iFile,iEdgeWeight) = trapz( ...
                    find(~isnan(meanRandRcCoefficient)), meanRandRcCoefficient( ...
                    ~isnan(meanRandRcCoefficient)));
		...
		elseif isFoundMultipleTimes
			...
			try            
				...
				fprintf(['Error unzipping file at ', pathToZip, ...
					...
					ME.stack(1).line, ME.stack(1).name, ME.message);
				fprintf('File "aseg.stats" not found in subject dirctory %s', pathToFa, ...
					'\n Nor was it found at %s', pathToZip, 'FreeSurfer/mri/aparc+aseg.mgz', ...
					'\n in zipped form. Please correct variable PATH_TO_FREESURFER_DIR', ...
					' or set FLAG_CALCULATE_COVARIATES to false.');
			end
